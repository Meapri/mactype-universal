/*
 * MacType Override System - Modernized
 *
 * Original code from 2006 has been modernized to use:
 * - Modern C++ memory management (std::vector instead of _alloca)
...
#include "supinfo.h"
#include "undocAPI.h"
#include "modern_cpp.h"  // Modern C++ utilities

#include <mbctype.h>	// _getmbcp

// Modern debug utilities
namespace debug_utils {
    ...
    // Modern debug logging
    inline void logDebugInfo(std::string_view message) {
#ifndef NDEBUG
        OutputDebugStringA(message.data());
#endif
    }

    inline void logDebugInfo(std::wstring_view message) {
#ifndef NDEBUG
...
// Modern constants and types
namespace override_constants {
    constexpr int CCH_MAX_STACK = 256;
}

// Modern function types using std::function
namespace function_types {
    using D2D1CreateFactory_Fn = std::function<HRESULT(
        D2D1_FACTORY_TYPE factoryType,
        REFIID riid,
        const D2D1_FACTORY_OPTIONS* pFactoryOptions,
        void** ppIFactory)>;

    using DWriteCreateFactory_Fn = std::function<HRESULT(
...
// Modern override system manager
class OverrideManager {
private:
    CFontCache fontCache_;
    CDCArray dcArray_;
    CDCRelationCache dcRelation_;
    CDCHwndCache dcHwndCache_;
    CDCRefCache dcRef_; // HDC reference count
    std::wstring nullString_;
    bool renderEnabled_ = true;
    bool cacheEnabled_ = true;
    HFONT alterGUIFont_ = nullptr;
    HFONT alterSysFont_ = nullptr;

    // Thread-safe singleton
    static std::unique_ptr<OverrideManager>& instance() {
        static std::unique_ptr<OverrideManager> instance_;
        static std::once_flag onceFlag_;

        std::call_once(onceFlag_, []() {
            instance_ = std::make_unique<OverrideManager>();
        });
...
public:
    ...
    OverrideManager(OverrideManager&&) = delete;
    OverrideManager& operator=(OverrideManager&&) = delete;

    // Access global state through singleton
    static OverrideManager& getInstance() {
        return *instance();
    }

    // Accessors for global state
    [[nodiscard]] CFontCache& fontCache() noexcept { return fontCache_; }
    [[nodiscard]] CDCArray& dcArray() noexcept { return dcArray_; }
    [[nodiscard]] CDCRelationCache& dcRelation() noexcept { return dcRelation_; }
    [[nodiscard]] CDCHwndCache& dcHwndCache() noexcept { return dcHwndCache_; }
    [[nodiscard]] CDCRefCache& dcRef() noexcept { return dcRef_; }
    [[nodiscard]] std::wstring& nullString() noexcept { return nullString_; }
    [[nodiscard]] bool renderEnabled() const noexcept { return renderEnabled_; }
    [[nodiscard]] bool cacheEnabled() const noexcept { return cacheEnabled_; }
    [[nodiscard]] HFONT alterGUIFont() const noexcept { return alterGUIFont_; }
    [[nodiscard]] HFONT alterSysFont() const noexcept { return alterSysFont_; }

    // Mutators
    void setRenderEnabled(bool enabled) noexcept { renderEnabled_ = enabled; }
    void setCacheEnabled(bool enabled) noexcept { cacheEnabled_ = enabled; }
    void setAlterGUIFont(HFONT font) noexcept { alterGUIFont_ = font; }
    void setAlterSysFont(HFONT font) noexcept { alterSysFont_ = font; }
...
CFontCache& FontCache = OverrideManager::getInstance().fontCache();
CDCArray& DCArray = OverrideManager::getInstance().dcArray();
CDCRelationCache& DCRelation = OverrideManager::getInstance().dcRelation();
CDCHwndCache& DCHwndCache = OverrideManager::getInstance().dcHwndCache();
CDCRefCache& DCRef = OverrideManager::getInstance().dcRef();
std::wstring& nullstring = OverrideManager::getInstance().nullString();
BOOL& g_ccbRender = reinterpret_cast<BOOL&>(OverrideManager::getInstance().renderEnabled());
BOOL& g_ccbCache = reinterpret_cast<BOOL&>(OverrideManager::getInstance().cacheEnabled());
HFONT& g_alterGUIFont = OverrideManager::getInstance().alterGUIFont();
HFONT& g_alterSysFont = OverrideManager::getInstance().alterSysFont();

//BOOL FreeTypeGetTextExtentPoint(HDC hdc, LPCSTR lpString, int cbString, LPSIZE lpSize, const FREETYPE_PARAMS* params);

// Modern font retrieval with better error handling
[[nodiscard]] HFONT GetCurrentFont(HDC hdc) {
    if (!hdc) {
        return nullptr;
    }
    ...
    if (!hCurFont) {
        // Fallback to system font if current font is not available
        hCurFont = static_cast<HFONT>(GetStockObject(SYSTEM_FONT));
    }
    return hCurFont;
}

// Check if DC is valid for display rendering (skips subtitles)
[[nodiscard]] bool IsValidDC(HDC hdc) {
    if (!hdc) {
        return false;
    }

    // Only process raster display DCs
