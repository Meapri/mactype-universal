/*
 * MacType FreeType Engine - Modernized
 *
...
#include <windows.h>
#include <cmath>  // Modern C++ math functions
...
// Modern constants and utilities
namespace ft_constants {
    constexpr int FT_BOLD_LOW = 15;
    constexpr int RESOLUTION_X = 72;
    constexpr int RESOLUTION_Y = 72;

    // Modern inline functions instead of macros
    [[nodiscard]] constexpr bool IsFontBold(const LOGFONTW& lf) noexcept {
        return lf.lfWeight >= FW_BOLD;
    }

    [[nodiscard]] constexpr int FT_FixedToInt(FT_Fixed x) noexcept {
        return (FT_RoundFix(x) >> 16);
    }

    [[nodiscard]] constexpr int FT_PosToInt(FT_Pos x) noexcept {
        return ((x + (1 << 5)) >> 6);
    }
}

FT_BitmapGlyphRec empty_glyph = {}; // Optimization control
...
namespace color_utils {
    [[nodiscard]] constexpr COLORREF RGBA(BYTE r, BYTE g, BYTE b, BYTE a = 255) noexcept {
    ...
    [[nodiscard]] constexpr COLORREF RGB(BYTE r, BYTE g, BYTE b) noexcept {
...
{
    // Modern version with better error handling
    ...
    PALETTEENTRY entry = {};
    const UINT index = paletteindex & 0xffff;

    if (GetPaletteEntries(hpal, index, 1, &entry) != 1) {
        return CLR_INVALID;
    }

    return color_utils::RGB(entry.peRed, entry.peGreen, entry.peBlue);
}

bool EmBoldVariableFont(const FT_Face face, int boldWeight) {
    // Modern RAII-based resource management
    struct MM_Var_Deleter {
        void operator()(FT_MM_Var* ptr) const noexcept {
            if (ptr) {
                FT_Done_MM_Var(freetype_library, ptr);
            }
        }
    };

    using unique_mm_var = std::unique_ptr<FT_MM_Var, MM_Var_Deleter>;

    FT_MM_Var* raw_mm_var = nullptr;
    const auto error = FT_Get_MM_Var(face, &raw_mm_var);
    if (error) {
        return false; // not a variable font
    }

    unique_mm_var mm_var(raw_mm_var);

    const auto num_axes = mm_var->num_axis;
    if (num_axes == 0) {
        return false;
    }

    // Modern vector for coordinates (RAII memory management)
    std::vector<FT_Fixed> coords(static_cast<size_t>(num_axes));
    bool axisFound = false;

    // Find 'wght' axis and set coordinates
    for (size_t i = 0; i < static_cast<size_t>(num_axes); ++i) {
        const FT_Var_Axis& axis = mm_var->axis[i];
        if (axis.tag == FT_MAKE_TAG('w', 'g', 'h', 't')) {
            axisFound = true;
            // Fix out-of-range bold weight using modern C++ utilities
            const auto clampedWeight = std::clamp(boldWeight,
                                                 axis.minimum,
                                                 axis.maximum);
            coords[i] = clampedWeight;
        } else {
    ...
    return true; // RAII will automatically clean up mm_var
}
...
namespace logging {
    ...
    // Modern file-based logging
    void Log(std::string_view message) {
#ifndef _DEBUG
        return;
#endif
        // Modern RAII-based file handling
        std::ofstream logFile(".\\gdipp.log", std::ios::app);
        if (logFile) {
            logFile << message;
        }
    }
    ...
    void Log(std::wstring_view message) {
        ...
        const auto utf8Message = string_utils::utf16_to_utf8(message);
        Log(utf8Message);
    }
...
// Legacy compatibility function for wide strings
void Log(wchar_t* Msg) {
    if (Msg) {
        logging::Log(std::wstring_view(Msg));
    }
}

// Removed custom FT_Glyph_To_BitmapEx dependency - using standard FreeType API
			...
			if ((*glyph_bitmap)->ft_glyph->format != FT_GLYPH_FORMAT_BITMAP) {
				...
				}
				{
					...
					if (bLoadColor && FT_HAS_COLOR(freetype_face)) {
						// Use standard FreeType API - color information is already loaded in the glyph
						// The custom FT_Glyph_To_BitmapEx is not strictly necessary as the color data
						// is preserved during standard bitmap conversion
						FT_Error glyph_error = FT_Glyph_To_Bitmap(&((*glyph_bitmap)->ft_glyph), render_mode, 0, 1);
