/*
 * MacType Modern API Hooking System
 *
 * Modernized Windows API hooking using:
...
// Modern module instance management
inline HINSTANCE& GetModuleInstance() noexcept {
...
// Modern function pointer types using std::function
namespace function_types {
    using CreateProcessW_Fn = std::function<BOOL(
        LPCWSTR, LPWSTR, LPSECURITY_ATTRIBUTES, LPSECURITY_ATTRIBUTES,
        BOOL, DWORD, LPVOID, LPCWSTR, LPSTARTUPINFOW, LPPROCESS_INFORMATION)>;

    using CreateProcessA_Fn = std::function<BOOL(
        LPCSTR, LPSTR, LPSECURITY_ATTRIBUTES, LPSECURITY_ATTRIBUTES,
        BOOL, DWORD, LPVOID, LPCSTR, LPSTARTUPINFOA, LPPROCESS_INFORMATION)>;
}

// Modern library loader utility
class LibraryLoader {
private:
    HMODULE handle_;

public:
    explicit LibraryLoader(std::string_view libName) noexcept
        : handle_(LoadLibraryA(libName.data())) {}

    explicit LibraryLoader(std::wstring_view libName) noexcept
        : handle_(LoadLibraryW(libName.data())) {}
    ...
    template<typename T>
    [[nodiscard]] T getProcAddress(std::string_view procName) const noexcept {
        if (!handle_) return nullptr;
        return reinterpret_cast<T>(GetProcAddress(handle_, procName.data()));
    }
};
...
// Modern hook function management
namespace hook_system {

    // Modern function pointer wrapper with RAII
    template<typename T>
    ...
    public:
        explicit FunctionPointer(T* original = nullptr, T* hook = nullptr) noexcept
            : original_(original), hook_(hook) {}

        ~FunctionPointer() noexcept {
            unhook();
        }

        // Prevent copying
        FunctionPointer(const FunctionPointer&) = delete;
        FunctionPointer& operator=(const FunctionPointer&) = delete;

        // Allow moving
        FunctionPointer(FunctionPointer&& other) noexcept
            : original_(std::exchange(other.original_, nullptr))
            , hook_(std::exchange(other.hook_, nullptr))
            , hooked_(std::exchange(other.hooked_, false)) {}

        FunctionPointer& operator=(FunctionPointer&& other) noexcept {
            if (this != &other) {
                unhook();
                original_ = std::exchange(other.original_, nullptr);
                hook_ = std::exchange(other.hook_, nullptr);
        ...
        [[nodiscard]] bool isHooked() const noexcept { return hooked_; }

        bool hook() noexcept {
            if (!original_ || !hook_ || hooked_) {
                return false;
            }

            if (DetourAttach(&(PVOID&)original_, hook_) == NOERROR) {
                hooked_ = true;
                return true;
            }
            return false;
        ...
        bool unhook() noexcept {
            if (!hooked_ || !original_ || !hook_) {
                return false;
            }

            if (DetourDetach(&(PVOID&)original_, hook_) == NOERROR) {
                hooked_ = false;
                return true;
            }
            return false;
        ...
        [[nodiscard]] T* getOriginal() const noexcept { return original_; }
    };
...
// Modern hook registry and initialization
namespace hook_initialization {

    class HookRegistry {
    private:
        std::vector<std::function<bool()>> hookInstallers_;
        std::vector<std::function<bool()>> hookUninstallers_;
    ...
    public:
        ...
        void registerHook(std::function<bool()> installer, std::function<bool()> uninstaller = nullptr) {
            hookInstallers_.push_back(std::move(installer));
            if (uninstaller) {
                hookUninstallers_.push_back(std::move(uninstaller));
            }
        }

        bool installAllHooks() {
            bool success = true;
            for (auto& installer : hookInstallers_) {
                if (!installer()) {
                    success = false;
                    break;
                }
    ...
    static LONG hook_init() {
        ...
        // Modern hook registry integration
        auto& registry = HookRegistry::instance();

        // Legacy hooklist.h integration (will be modernized separately)
        #include "hooklist.h"

	LONG error = DetourTransactionCommit();

        if (error != NOERROR) {
            // Modern logging instead of TRACE macro
            std::stringstream ss;
            ss << "hook_init error: 0x" << std::hex << error;
