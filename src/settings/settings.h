/*
 * MacType Settings System - Modernized
 *
 * Modern configuration management with:
 * - JSON-based settings
...
// Modern string conversion utilities (using modern_cpp.h)
namespace string_conversions {

    // UTF-8/UTF-16 conversion using modern_cpp utilities
    [[nodiscard]] inline std::string to_utf8(std::wstring_view wstr) {
        return string_utils::utf16_to_utf8(wstr);
    }

    [[nodiscard]] inline std::wstring to_wstring(std::string_view str) {
        return string_utils::utf8_to_utf16(str);
    }
...
// JSON 기반 설정 시스템 (nlohmann/json 라이브러리 사용)
